...
@section('content')
    ...
    <form action="{{route('roles.update', ['role' => $role->id])}}" id="form_advanced_validation" method="post">
                @method('put')
                @csrf
                @component('components.input',['label' => 'Name', 'type'  => 'text','name'  => 'name', 'value' => $role ->name])@endcomponent
                <!-- permissions -->
                                   ...
                                   <div class="card">
                                       <div class="body p-0">
                                        ...
                                        @foreach($permission as $method)
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" {{in_array($key . '.' . $method, (old('permissions') ? old('permissions') : $rolePermissions)) ? 'checked="checked"' :''}} class="custom-control-input {{$key}}"
                                                       name="permissions[]" value="{{$key . '.' . $method}}" id="permission-{{$key . '_' .$method}}"  />
                ...
                <div class="container text-center">
                    <button type="submit" class="btn btn-primary mr-2 special_button">save</button>
                </div>
            </form>
