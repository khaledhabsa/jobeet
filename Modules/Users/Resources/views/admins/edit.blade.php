...
@section('content')
    ...
    <div class="row clearfix">
        <div class="col-lg-12 col-md-12 col-sm-12">
            <form action="{{route('admins.update',['admin'=>$admin->id])}}" id="form_advanced_validation" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                ...
                @component('components.select',['label' => 'Role', 'name'=> 'role_id', 'classes' => 'select2'])
                    @slot('options')
                        @foreach($roles as $r)
                            @if(old('role'))
                                <option value="{{$r->name}}" {{$r->name == old('role') ? 'selected="selected"' : ''}}>{{$r->name}}</option>
                            @else
                                <option value="{{$r->name}}" {{$admin->hasRole($r->name) ? 'selected="selected"' : ''}}>{{$r->name}}</option>
                            @endif
                        @endforeach
                    @endslot
                @endcomponent
